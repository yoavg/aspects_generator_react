...
export async function get_title(corpusId: number): Promise<string> {
  const response = await fetch("/api/title", { method: "POST", body: JSON.stringify({ corpusId }), headers: { "Content-Type": "application/json" } });
